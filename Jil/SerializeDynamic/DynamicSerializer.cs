   ...
   class DynamicSerializer
   {
      ...
      private static void SerializeInternal(MemberInfo dynamicMember, TextWriter stream, object obj, Options opts, int depth)
      {
         ...
         if (dynObject != null && !(canTreatExpandoAsDictionary && dynObject is ExpandoObject))
         {
            ...
            DateTimeOffset dto;
            if (!opts.ShouldNotCoerceToDateOnSerialize && CanBeDateTimeOffset(dynObject, out dto))
            {
            ...
            DateTime dt;
            if (!opts.ShouldNotCoerceToDateOnSerialize && CanBeDateTime(dynObject, out dt))
            {
            ...
            TimeSpan ts;
            if (!opts.ShouldNotCoerceToDateOnSerialize && CanBeTimeSpan(dynObject, out ts))
            {
