...
namespace JilTests
{
    ...
    public class SpeedProofTests
    {
        ...
        class _CacheDynamicExpressions
        {
            public int A { get; set; }
            public string B { get; set; }
            public int[] C { get; set; }
            public _CacheDynamicExpressions D { get; set; }

            public static _CacheDynamicExpressions Get(Random rand)
            {
                var sub = new int[rand.Next(10)];
                ...
                return 
                    new _CacheDynamicExpressions
                        {
                            A = rand.Next(),
                            B = _RandString(rand),
                            C = sub,
                            D = rand.Next(2) == 1 ? Get(rand) : null
                        };
            }
        }

        [TestMethod]
        public void CacheDynamicExpressions()
        {
            try
            {
                Func<TextReader, _CacheDynamicExpressions> fast = null;
                Func<TextReader, _CacheDynamicExpressions> normal = null;

                fast =
                    txt =>
                    {
                        Jil.DeserializeDynamic.JsonObject.CacheDynamicExpressions = true;

                        var resp = Jil.JSON.DeserializeDynamic(txt);

                        _CacheDynamicExpressions first = null;
                        _CacheDynamicExpressions ret = null;

                        do
                        {
                            int a = resp.A;
                            string b = resp.B;
                            int[] c = ((IEnumerable<int>)resp.C).ToArray();
                            dynamic d = resp.D;
                ...
                normal =
                    txt =>
                    {
                        Jil.DeserializeDynamic.JsonObject.CacheDynamicExpressions = false;
                ...
                var toSerialize = new List<_CacheDynamicExpressions>();
                for (var i = 0; i < 10000; i++)
                {
                    toSerialize.Add(_CacheDynamicExpressions.Get(rand));
                }
